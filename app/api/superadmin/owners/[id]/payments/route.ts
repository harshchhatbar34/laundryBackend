...
import { getOwnerPaymentHistory } from '@/src/modules/subscription/subscription.service';

// GET /api/superadmin/owners/[id]/payments
export const GET = withRole('superadmin')(async (_req: NextRequest, ctx: AuthContext<{ id: string }>) => {
  try {
    await connectDB();
    const payments = await getOwnerPaymentHistory(ctx.params.id);
